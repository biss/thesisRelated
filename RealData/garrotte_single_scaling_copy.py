...
''' SINGLE scaling factor
    ...
    CHANGES: RMS error is calculated without the membership values.
...
class SwitchingRegression_ss:
    ...
    def __init__(self, cluster=2 ):
        ...
        data = np.genfromtxt('input/kuiper_ip_1.csv', delimiter=',', skip_header=1)
    ...
    def train(self, l):
        ...
        rms_1 = math.sqrt(mse(np.dot(X_1, self.regime_param[0]) ,Y_1) + mse(np.dot(X_2, self.regime_param[0]) ,Y_2))
        rms_2 = math.sqrt(mse(np.dot(X_2, self.regime_param[0]) ,Y_2) + mse(np.dot(X_1, self.regime_param[0]) ,Y_1))
        if(rms_1 > rms_2):
            self.rms = rms_2
        else:
            self.rms = rms_1
...
if __name__ == '__main__':
    ...
    for element in lmd:
        ...
        sr = SwitchingRegression_ss(clusters)
    ...
    with open('output/0304_result_garrote.csv', 'wb') as file:
