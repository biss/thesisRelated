...
class SwitchingRegression:
    ...
    def __init__(self):
        ...
        self.regime_param = [[0.0 for i in range(self.dim-1)], [0.0 for i in range(self.dim-1)]]
    ...
    def newModel(self, A, b, lm):
        ...
        if lm == 0.0:
            ...
            Q = matrix([[2*A.T*A, -2*A.T*A],[-2*A.T*A, 2*A.T*A]])
...
if __name__ == '__main__':
    ...
    for element in lmd:
        ...
        rms[element] = [sr.err, np.matrix.sum(np.matrix(sr.U*sr.E)), sr.crisp_obj]
    print rms

    with open('output/fcrm_with_l1.synthetic.cor_data.csv', 'wb') as file:
