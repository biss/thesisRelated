...
''' -----------------------------------------------------------------
    Data file used: input/out_n.csv
    C's: separate set of c's are used for each models
...
class SwitchingRegression_g:
    ...
    belittle = np.zeros(shape = (c, features - 1), dtype=float)
    ...
    def load_data(self):
        ...
        data = np.genfromtxt('input/out_original.csv', dtype=float, delimiter=',')
        ...
        self.betas[0, :] = np.array([[1.17, 0.209, -0.055, 0.021, 0.035]])
        self.betas[1, :] = np.array([[.56, .096, .0023, -0.000003, 0.52]])
...
if __name__ == '__main__':
    ...
    for element in lmd:
        ...
        res[element] = [sr.regime_param, sr.obj, sr.abs_v, sr.belittle, np.array([[sr.rms/sr.number]])]
    ...
    with open('output/result.fcrm_garrotte.synthetic.unnorm.separate_c.csv', 'wb') as file:
