...
''' Last Edited: 16th Arp 2016
    ...
    x1 and x5 are linearly dependent x5 = x1 + gaussian noise(0 mean, unit stdev).
'''
...
class SwitchingRegression:
    ...
    def generateData(self):
        ...
        noise_1 = list((x + y) for x,y in zip(list(x*2.2 for x in imp_1), a1))
        ...
        noise_2 = list((x + y) for x,y in zip(list(x*2.2 for x in imp_2), b1))
        ...
        np.savetxt("input/out_unnorm_mulcolinear.csv", self.data, delimiter=",")

    def load_data(self):
        self.data = genfromtxt('input/out_unnorm_mulcolinear.csv', delimiter=',')
        print self.data
