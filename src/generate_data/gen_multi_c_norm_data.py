...
''' Last Edited: 16th Apr 2016
    generate normalized data containing a multicolinear column.
    DATA FORMAT::: each column of data is a data point, [x1, x2, x3, x4, x5, y].T is a column
...
class SwitchingRegression:
    ...
    def generateData(self):
        ...
        noise_1 = list((x + y) for x,y in zip(list(x*2.2 for x in imp_1), list(np.random.normal(0, 1.0, self.number/2))))

        x_regime1 = np.array([imp_1, [np.random.uniform(-5,5) for i in range(self.number/2)], [np.random.uniform(-5,5) for i in
        ...
        y_axis_regime1 = np.column_stack(np.array([a_1, b_1, 0, d_1])).dot(x_regime1[:4, :])
        #1  print y_axis_regime1.shape

        #adding noise to the dependent variable
        noise_y = np.random.normal(0, 1.0, self.number / 2)
        y_axis_regime1 = y_axis_regime1 + noise_y
        ...
        noise_2 = list((x + y) for x,y in zip(list(x*2.2 for x in imp_2), list(np.random.normal(0, 1.0, self.number/2))))

        x_regime2 = np.array([imp_2, [np.random.uniform(-5,5) for i in range(self.number/2)], [np.random.uniform(-5,5) for i in
                                range(self.number/2)], [np.random.uniform(-5,5) for i in range(self.number/2)], noise_2])

        y_axis_regime2 = np.column_stack(np.array([a_2, b_2, c_2, 0])).dot(x_regime2[:4, :])

        #adding noise to the dependent variable
        y_axis_regime2 = y_axis_regime2 + np.random.normal(0, 1.0, self.number / 2)
        ...
        np.savetxt("../input/out_m_n.csv", self.data, delimiter=",")
