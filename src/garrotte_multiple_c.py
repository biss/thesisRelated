...
class SwitchingRegression_g:
    ...
    def __init__(self, cluster=2):
        ...
        self.data = np.genfromtxt('input/cor_data.csv', dtype=float, delimiter=',')
        ...
        self.betas[0, :] = np.array([[1.31, 0.23, -0.13, -0.0001, 0.001, 0.01, -0.003, -0.04]])
        self.betas[1, :] = np.array([[0.53, 0.14, -0.001, 0.003, -0.15, 0.0008, -0.02, 0.20]])
    ...
    def train(self, l):
        ...
        print 'final objective function value: ', obj
...
if __name__ == '__main__':
    ...
    for element in lmd:
        ...
        rms[element] = [sr.err, np.matrix.sum(np.matrix(sr.E*sr.U)), sr.crisp_obj]

    #beta[i] = sr.regime_param
    #print res
    print rms

    with open('output/fcrm.garrotte.multiple_c.cor_data.csv', 'wb') as file:
        ...
        for row in zip(*res.values()):
            for row1 in zip(*row):
                ...
                writer.writerow('-')
    '''
    with open('result_output/single_wt_garrotte_fcrm.csv', 'wb') as file:
        writer = csv.writer(file, delimiter='\t')
